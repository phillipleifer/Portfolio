/* For this assignment, we will use JOINS to combine information about U.S. counties from multiple different sources. Here's an overview of the 
...
Start by loading each of these datasets into MySQL. NOTE: Some of these may take awhile to load, especially county_neighbors!

A few other data cleaning notes:
 - These dataset use each county's FIPS code, which sometimes starts with a 0. The CSV's have fips codes stored as text, and you can keep them as
 text when loading into MySQL. But don't open these CSV's in Excel, or it will interpret the fips codes as numbers and removed the leading zero.
 - After uploading your files, make sure FIPS codes are all saved as TEXT (or CHAR/VARCHAR), and years are all saved as INT. We will be using these columns
 to join tables, so we want to make sure their data types are consistent.
 - Note that the county columns is named 'fips' in povertyrates and 'countyfips' in unemployment. You can leave these columns as-is, or change one of the
 column names if you want to use the USING() keyword in your joins (this will probably make your life easier!).
 */

/* SECTION 1 (9 points): COMBINING POVERTY AND UNEMPLOYMENT RATES FOR EACH COUNTY */

/* 1a (2 points): You want a combined dataset that lists the poverty rate and unemployment rate for each U.S. county.
...
/*1b (1 points): Let's figured out how many COUNTIES (not rows) we 'lost' by an inner join?
...
/* 1c (2 point): You should have found that you 'lost' about 50ish counties from the povertyrates table, and about 80ish counties from the 
...
/* 1d (2 points):  Often when working with time series data, it's best to have rows for each year of interest, even if that means having missing data in those rows.
...
/* 1e (2 points): Lastly, let's left-join the date from 'joined' (povrate, unemployment) to 'county_year_combos'.
...
/* SECTION 2 (5 points): APPENDING STATE MEDICAID INFORMATION */

/* 2a (1 point): One important characteristic economists consider when analyzing county data is whether their state has expanded Medicaid, when is when states
...
/* 2b (2 points): Let's append the column 'medicaid_year' to our 'joined_final' table, using a LEFT JOIN. This will tell us, for each county, the year in which
...
/* 2c (2 point): Re-run the same query, but select one more column: year >= medicaid_year.
...
/* SECTION 3 (6 points): APPENDING INFORMATION ABOUT ADJACENT COUNTIES */
...
/* 3a (2 points): In the 'county_neighbors' table, you'll notice that each county is listed as being adjacent to itself.
...
/* 3b (4 points): This last query will be a bit challenging - consider this a 'final test' of how well you've mastered joins in this class!
...
#Be sure to export the result when you're done and upload to Canvas!